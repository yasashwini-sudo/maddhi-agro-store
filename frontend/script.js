...
if (user) {
userSection.innerHTML = `       <div style="display:flex; align-items:center; gap:10px;">         <span>👋 ${user.name}</span>         <button onclick="logout()">Logout</button>       </div>
    `;
} else {
userSection.innerHTML = `       <button onclick="openAuth()">Login</button>
    `;
}
}
